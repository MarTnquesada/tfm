...
for lang in de hi; do
  for method in base muse vecmap concat; do
    ...
    docker container run -it --rm -v ${PWD}/data/:/data moses /bin/bash -c "scripts/generic/multi-bleu.perl /data/datasets/parallel/${lang}-en.test.clean.bpe_48000.en < /data/models/eval/pred_${lang}-en-512.txt"
  done
done
...
for lang in fr de ru hi; do
  for method in base muse vecmap concat; do
    ...
    docker container run -it --rm -v ${PWD}/data/:/data moses /bin/bash -c "scripts/generic/multi-bleu.perl /data/datasets/parallel/${lang}-en.test.clean.en < /data/models/eval/pred_${lang}-en_512_${method}_unfrozen_bpe.txt"
  done
done