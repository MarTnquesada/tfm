...
for lang in fr de ru hi; do
  for method in base muse vecmap concat; do
    ...
    onmt_translate -model data/models/trained/${lang}-en_512_${method}_unfrozen/model_step_20000.pt -src data/datasets/parallel/${lang}-en.test.clean.${lang} -output data/models/eval/pred_${lang}-en_512_${method}_unfrozen.txt -gpu 0
    docker container run --rm -v ${PWD}/data/:/data moses /bin/bash -c "scripts/generic/multi-bleu.perl /data/datasets/parallel/${lang}-en.test.clean.en < /data/models/eval/pred_${lang}-en_512_${method}_unfrozen.txt"
  done
done